...
<div class="notice">
  <h3>Running on free models, so this is slow</h3>
  <p style="margin:0">
    The models here are on a free tier that limits how many requests can be made
    per minute, and the pipeline waits rather than being refused. An analysis
    takes a while, and the daily allowance is not generous enough to finish many
    of them. A paid key removes the limit and the same run finishes considerably
    faster. Switching is one line in <code>.env</code>: the provider and the
